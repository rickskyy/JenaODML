...
from query_handler.services.format import Document
from query_handler.services.format import Property
from query_handler.services.format import Section
...
class QueryCreator(BaseQueryCreator):
    ...
    def get_query(self, dataset=None, q_str=None, q_parser=None):
        """
        ...
        return self._prepare_query(dataset)

    def _prepare_query(self, dataset=None):
        """
        Creates query using parameters from self.q_dict.
        ...
        # TODO enable adding prefixes from query parameters
        prefixes = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " \
                   "PREFIX owl: <http://www.w3.org/2002/07/owl#> " \
                   "PREFIX odml: <https://g-node.org/projects/odml-rdf#> "

        # FIXME modify architecture -> e.g. decorator pattern
        self.query = prefixes
        self.query += 'SELECT * '
        if dataset:
            self.query += 'FROM ' + dataset
        self.query += ' WHERE { '
            ...
            if len(doc_attrs) > 0:
                ...
                for i in doc_attrs:
                    if len(i) > 2:
                        ...
                        if attr:
                            self.query += '?d {0} \"{1}\" . '.format(re.sub(odml_uri,
                                                                             "odml:", attr), i[1])
            ...
            if len(sec_attrs) > 0:
                ...
                for i in sec_attrs:
                    if len(i) > 2:
                        ...
                        if attr:
                            self.query += '?s {0} \"{1}\" . '.format(re.sub(odml_uri,
                                                                             "odml:", attr), i[1])
            ...
            if len(prop_attrs) > 0:
                ...
                for i in prop_attrs:
                    if len(i) > 2:
                        ...
                        values = i[1]
                        if values:
                            self.query += "?p odml:hasValue ?v . ?v rdf:type rdf:Bag . "
                            for v in values:
                                self.query += '?v rdf:li \"{}\" . '.format(v)
                    else:
                        attr = Property.rdf_map(i[0])
                        if attr:
                            self.query += '?p {0} \"{1}\" . '.format(re.sub(odml_uri,
                                                                             "odml:", attr), i[1])
