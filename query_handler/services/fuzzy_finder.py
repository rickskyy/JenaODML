from query_handler.services.query_creator import QueryCreator, QueryParser, QueryParserFuzzy
...
class FuzzyFinder(object):
    ...
    def find(self, dataset, mode='fuzzy', q_str=None, q_params=None,):
        # TODO warn users if they added non-odml attributes ('naming' instead of 'name' e.g.)
        """
        Build list of SPARQL queries from specified query string or query parameters
        ...
        :return:         list which contains set of queries.
        """
        if mode == 'fuzzy':
        ...
        self._validate_find_input_attributes(q_str, q_params, q_parser)

        self._generate_parameters_subsets(pairs_generator())

        return self._output_built_query_list(dataset)

    def _validate_find_input_attributes(self, q_str, q_params, q_parser):
    ...
    def _output_built_query_list(self, dataset):
        query_list = []
        ...
        for query in self._subsets:
            ...
            query_list.append(creator.get_query(dataset=dataset))
        return query_list
